...
export default function DoctorWorkspaceView({ user, initialSubTab = 'summary' }) {
  ...
  const [activeTab, setActiveTab] = useState(initialSubTab); // 'summary' | 'timeline' | 'prescription' | 'admission'
  ...
  const handleConfirmSummary = async () => {
    ...
    try {
      ...
      alert(`AI Summary confirmed and signed by ${doctorName}.`);
    } catch (e) {
      console.error(e);
    }
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0 0', fontSize: '14px', color: 'var(--text-muted)' }}>
            Attending Physician: <strong>{doctorName} ({user?.specialty_or_info || 'Chief of Medicine'})</strong> &bull; OPD & Triage Review
          </p>
        </div>
