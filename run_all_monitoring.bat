...
:: 1. Backend Server
echo [1/5] Starting FastAPI Backend Gateway & Telemetry Streaming Service...
cd /d "%~dp0backend"
...
:: 2. Pharmacy Console & Real-Time Inventory
echo [2/5] Starting AuraHealth Pharmacy Console & Formulary Hub (:5173)...
cd /d "%~dp0frontend"
...
:: 3. Patient Companion App
echo [3/5] Starting Patient Companion Portal (:5174)...
cd /d "%~dp0patient-app"
if not exist "node_modules\" (
...
:: 4. Receptionist Desk App
echo [4/5] Starting Receptionist Desk Portal (:5175)...
...
:: 5. Doctor Clinical Station App
echo [5/5] Starting Doctor Clinical Workstation (:5176)...
cd /d "%~dp0doctor-app"
if not exist "node_modules\" (
...
echo    [5] Backend Gateway API (Firebase):      http://127.0.0.1:8000
