...
export default function MediKiosk({ onComplete }) {
  ...
  const [newPatientForm, setNewPatientForm] = useState({
    name: '', age: '', gender: 'Male', phone: '', blood_group: 'B+', allergies: 'None known', abha_id: ''
  });
  ...
  const handleMicToggle = () => {
    // 1. If currently speaking TTS audio, stop it immediately
    if (audioRef.current) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/intake/session/${sessionId}/document`, {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
  ...
  const handleGenerateSummary = async () => {
    setLoading(true);
    try {
      const summaryRes = await fetch(`${API_BASE}/intake/session/${sessionId}/generate-summary`, {
        method: 'POST'
      });
      ...
      const completeRes = await fetch(`${API_BASE}/intake/session/${sessionId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '16px' }}>
      ...
      {step === 'IDENTIFY' && (
        <div className="card" style={{ padding: '32px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '15px', marginBottom: '28px' }}>
            Please identify yourself using your UHID, Patient ID, Registered Mobile, or ABHA Health ID.
          </p>
          ...
          {!isRegisteringNew ? (
            <div style={{ maxWidth: '600px', margin: '0 auto' }}>
              ...
              <div style={{ marginBottom: '24px', textAlign: 'left' }}>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>Demo Quick Profiles:</span>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '6px' }}>
                  {[
                    { id: 'PT-1001', name: 'Eleanor Vance (PT-1001)' },
                    { id: 'PT-1002', name: 'Rohan Mehta (PT-1002)' },
                    { id: 'PT-1004', name: 'Priya Singh (Emergency PT-1004)' },
                    { id: 'PT-1006', name: 'Sita Devi (AYUSH/Cardio PT-1006)' }
                  ].map((p) => (
                    <button
                      key={p.id}
                      onClick={() => {
                        setPatientLookupQuery(p.id);
                        setSelectedPatient({
                          patient_id: p.id,
                          uhid: `UHID-2026-${p.id.split('-')[1]}`,
                          name: p.name.split(' (')[0],
                          age: 58,
                          gender: 'Female',
                          blood_group: 'A+',
      ...
      {step === 'CONSENT' && (
        <div className="card" style={{ padding: '32px', maxWidth: '850px', margin: '0 auto' }}>
          ...
          {/* Audio Explanation Toggle for Low-Literacy / Elderly */}
      ...
      {/* STEP 4: INTAKE CATEGORY (ALLOPATHY vs AYUSH) */}
      {step === 'INTAKE_TYPE' && (
        <div className="card" style={{ padding: '32px', textAlign: 'center' }}>
      ...
      {/* STEP 5: ADAPTIVE CONVERSATIONAL HISTORY (VOICE + TOUCH) */}
      {step === 'HISTORY' && (
        <div className="card" style={{ padding: '32px' }}>
          ...
          {/* Touch-Friendly Predefined Choices */}
      ...
      {/* STEP 7: OFFICIAL OPD TOKEN & CLINICAL SUMMARY PASS */}
