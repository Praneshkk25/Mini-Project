...
export default function DischargeSummaryAssistantView() {
  ...
  const [riskClass, setRiskClass] = useState('Low Risk');
  ...
  const [followUpDept, setFollowUpDept] = useState('Cardiology');
  ...
  const handleGenerateSummary = () => {
    ...
    setTimeout(() => {
      ...
      alert('AI Discharge Summary synthesized successfully!');
    }, 1200);
  };
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0 }}>
            📋 AI Discharge Summary & EHR Assistant
          </h1>
          <p style={{ margin: '4px 0 0 0', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Fill clinical details on the left — real-time hospital document preview updates automatically on the right.
          </p>
        </div>
        <div style={{ background: 'rgba(2, 132, 199, 0.12)', border: '1px solid var(--primary)', padding: '8px 16px', borderRadius: '12px', fontSize: '13px', color: 'var(--primary)' }}>
          🧠 Active Model: <strong>Qwen 2.5 (Local LLM)</strong>
        </div>
      </div>
        ...
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label className="form-label">AI Risk Classification</label>
                  <select 
                    value={riskClass}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <input type="text" placeholder="Adm (160/95)" value={vitals.bpAdm} onChange={e => setVitals({...vitals, bpAdm: e.target.value})} className="form-input" />
                  <input type="text" placeholder="Dis (122/78)" value={vitals.bpDis} onChange={e => setVitals({...vitals, bpDis: e.target.value})} className="form-input" />
                </div>
              </div>
              <div>
          ...
          <div>
            ...
            <div>
              <label className="form-label">Raw Clinical Doctor Notes</label>
              <textarea 
                rows={4}
                value={rawNotes}
